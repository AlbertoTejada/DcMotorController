...
/*
Check if there is space in the buffer and writes a byte at the end,
if the end of the buffer is reached, it goes to the possition 0
*/
...
void serial_write(uint8_t data) {
  ...
  // Wait until there is space in the buffer (see the interruption handler)
  while (next_head == tx_buffer_tail) { 
    //TODO: adapt this to the project
...
/*
Takes a byte from the tx_buffer and copy it in the UART
registry when it is available, after that, moves fordward the 
tail of the buffer and checks if it is empty to disable
the interruption
*/
...
/*
Takes a byte from the rx_buffer, if it is empty,
returns SERIAL_NO_DATA
*/
void serial_read(uint8_t* data, uint8_t* serial_avail)
{
  if (rx_buffer_head == rx_buffer_tail) {
    *data = SERIAL_NO_DATA;
    *serial_avail = SERIAL_NOT_AVAIL;
  } else {
...
#ifndef TESTING
/*
Populates the rx_buffer when there is a reception on the UDR0
registry, if the buffer is full, the data received is lost
*/
#if defined(__AVR_ATmega644P__) || defined(__AVR_ATmega2560__)
ISR(USART0_RX_vect)
#else
